...
function addSubcategories(category) {
    if (category!="") {
        $.ajax({
            url: '/misterfix/subcategorias/'+category+'/agregar',
            dataType: 'html',
        })
        .done(function(resultado) {
            $("#multiselectSubcategories option").remove();
            var obj=JSON.parse(resultado);

            $('#multiselectSubcategories').append($('<option>', {
                value: '',
                text: 'Seleccione'
            }));
            ...
            $('#multiselectSubcategories').multiselect('rebuild');
        });
    } else {
        $("#multiselectSubcategories option").remove();
        $('#multiselectSubcategories').multiselect('rebuild');
...
function deleteUser(slug) {
    $("#deleteUser").modal();
    $('#formDeleteUser').attr('action', '/misterfix/usuarios/' + slug);
}

function deleteCategory(slug) {
    $("#deleteCategory").modal();
    $('#formDeleteCategory').attr('action', '/misterfix/categorias/' + slug);
}

function deleteSubcategory(slug) {
    $("#deleteSubcategory").modal();
    $('#formDeleteSubcategory').attr('action', '/misterfix/subcategorias/' + slug);
}

function deleteStore(slug) {
    $("#deleteStore").modal();
    $('#formDeleteStore').attr('action', '/misterfix/tiendas/' + slug);
}

function deleteProduct(slug) {
    $("#deleteProduct").modal();
    $('#formDeleteProduct').attr('action', '/misterfix/productos/' + slug);
}

function deleteBrand(slug) {
    $("#deleteBrand").modal();
    $('#formDeleteBrand').attr('action', '/misterfix/marcas/' + slug);
}

function deleteBlog(slug) {
    $("#deleteBlog").modal();
    $('#formDeleteBlog').attr('action', '/misterfix/blog/' + slug);
}

//Funciones para mostrar modales para confirmar o rechazar
function confirmPay(slug) {
    $("#confirmPay").modal();
    $('#formConfirmPay').attr('action', '/misterfix/ventas/confirmar/' + slug);
}

function refusePay(slug) {
    $("#refusePay").modal();
    $('#formRefusePay').attr('action', '/misterfix/ventas/rechazar/' + slug);
}

function confirmStore(slug) {
    $("#confirmStore").modal();
    $('#formConfirmStore').attr('action', '/misterfix/tiendas/confirmar/' + slug);
}

function refuseStore(slug) {
    $("#refuseStore").modal();
    $('#formRefuseStore').attr('action', '/misterfix/tiendas/rechazar/' + slug);
}

//Funcion para eliminar imagenes de productos
$('.btn-absolute-right').click(function(event) {
    var img=$(this).attr('image');
    var productSlug=$('#slug').val();
    var urlImage=event.currentTarget.attributes[3].value;
    urlImage=urlImage.split('/');
    $.ajax({
        url: '/misterfix/imagenes/productos/'+productSlug+'/'+urlImage[6]
    })
    .done(function(resultado) {
        var obj=JSON.parse(resultado);
        if (obj.status=='ok') {
